...
\def\AbstractEN{
    ...
    dataset consists of artificially generated data from a spiking model of 
    the cat's V1. We train our model to replicate neuronal responses in
    layer IV (LIV) and layers II and III (LII/III) of V1 based on input from the lateral geniculate nucleus (LGN). 
    ...
    incorporating more complex activation functions through the DNN module improves the model performance.
...
\def\AbstractCS{%
    ...
    našeho modelu cílíme na replikaci neuronálních odpovědí ve vrstvě IV (LIV) 
    a vrstvách  II a III (LII/III) V1 na základě vstupů z laterálního genikulárního jádra
    (LGN). Výsledky naší studie ukazují mají silnou shodu mezi oběma modely 
    vzhledem ke korelaci neuronových odpovědí a časoprostorové dynamice. 
